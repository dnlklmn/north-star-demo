...
export interface Seed {
   ...
   *  compatibility — older seeds loaded from DB may not have this field. */
...
export interface SessionState {
   ...
   *  discard. While set, seed.task.skill_body mirrors the candidate so
   ...
   *  Keys: 'goals' | 'users' | 'stories' | 'seed' | 'dataset' | 'scorers'. */
...
export interface EvalRunSummary {
  ...
  /** Full seed at the moment the run started. Null for older runs
   *  created before this column existed. */
  seed_snapshot?: Seed | null
...
export type SkillVersionSource = 'import' | 'suggestion' | 'manual' | 'restore'
