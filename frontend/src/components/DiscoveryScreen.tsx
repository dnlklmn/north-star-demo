...
export default function DiscoveryScreen({
  ...
  const [newStory, setNewStory] = useState<ExtractedStory>({
    who: "",
    what: "",
    why: "",
  });
  ...
  const isEmpty =
    goals.length === 0 && users.length === 0 && stories.length === 0;
      ...
      {isEmpty && phase === "goals" ? (
        <div className="flex flex-col items-center justify-center h-full text-center text-muted-foreground">
          ...
          <p className="text-sm max-w-md">
            The agent will help you define your business goals. As you talk,
            they'll appear here — or add them directly.
          </p>
        </div>
      ) : (
        <div className="space-y-8 max-w-2xl">
          ...
          <section
            className={
              isPastPhase("goals", phase)
                ? "opacity-60"
                : isFuturePhase("goals", phase)
                  ? "opacity-40"
                  : ""
            }
          >
            <div className="flex items-center justify-between mb-3">
              <h2 className="text-sm font-semibold text-muted-foreground uppercase tracking-wider">
                ...
                {isPastPhase("goals", phase) && goals.length > 0 && (
                  <span className="ml-2 text-xs font-normal normal-case text-accent">
                    done
                  </span>
                )}
              </h2>
            ...
            <div className="space-y-2">
              {goals.map((goal, i) => (
                <div
                  key={i}
                  className="group flex items-start gap-2 p-3 bg-surface hover:bg-surface-raised transition-colors"
                >
                  {editingGoal === i ? (
                    <input
                      autoFocus
              ...
              {showAddGoal && (
                <div className="flex items-start gap-2 p-3 bg-surface border border-accent/30">
                  <input
                    autoFocus
            ...
            {phase === "goals" && goals.length > 0 && (
              <button
                ...
                disabled={advancing}
                className="mt-4 flex items-center gap-2 px-4 py-2 bg-accent text-white text-sm font-medium hover:opacity-90 disabled:opacity-50 transition-opacity"
              >
          ...
          <section
            className={isFuturePhase("stories", phase) ? "opacity-40" : ""}
          >
            <div className="flex items-center justify-between mb-3">
              <h2 className="text-sm font-semibold text-muted-foreground uppercase tracking-wider">
                User Stories
            ...
            <div className="space-y-4">
              ...
              {(() => {
                  ...
                  users.length > 0
                    ? users.map((user) => ({
                        ...
                        stories: stories
                          ...
                          .filter(
                            ({ story }) =>
                              story.who.toLowerCase() === user.toLowerCase(),
                          ),
                      }))
                    : [];
                ...
                const matchedIndices = new Set(
                  userGroups.flatMap((g) => g.stories.map((s) => s.index)),
                );
              ...
              {showAddStory && (
                <div className="p-3 bg-surface border border-accent/30 space-y-2">
                  <div className="flex items-center gap-2">
                    ...
                    <input
                      ...
                      value={newStory.who}
                      onChange={(e) =>
                        setNewStory({ ...newStory, who: e.target.value })
                      }
                    />
                  </div>
                  <div className="flex items-center gap-2">
                    ...
                    <input
                      ...
                      value={newStory.what}
                      onChange={(e) =>
                        setNewStory({ ...newStory, what: e.target.value })
                      }
                    />
                  </div>
                  <div className="flex items-center gap-2">
                    ...
                    <input
                      ...
                      value={newStory.why}
                      onChange={(e) =>
                        setNewStory({ ...newStory, why: e.target.value })
                      }
                      onKeyDown={(e) => {
                        if (
                          e.key === "Enter" &&
...
function StoryItem({
          ...
          <input
            ...
            onKeyDown={(e) => {
              if (e.key === "Enter") {
                ...
                if (container) {
                  const whoInput = container.querySelector(
                    '[data-field="who"]',
                  ) as HTMLInputElement | null;
                  const whatInput = container.querySelector(
                    '[data-field="what"]',
                  ) as HTMLInputElement;
                  const whyInput = container.querySelector(
                    '[data-field="why"]',
                  ) as HTMLInputElement;
                  onSaveEdit({
  ...
  return (
    <div className="group flex items-start gap-2 p-3 bg-surface hover:bg-surface-raised transition-colors">
      <button
        onClick={onClick}
