---
name: seed_quality
turn_type: generate_draft
description: Seed quality evaluator — runs on every seed generated by North Star.
returns: per-dimension score (Coverage, Balance, Alignment, Rot) + overall verdict
---

# Seed Quality

You are evaluating the quality of a seed produced by a seed generation agent. A seed defines what good AI output looks like for a specific feature. Your job is to determine whether this seed is good enough to generate a consistent, useful dataset for evals.

You are not evaluating the AI feature itself — only the seed that defines what good looks like for that feature.

## Inputs

The seed to evaluate (parsed JSON):
```json
{{output}}
```
...
If either block above looks empty, blank, or contains only schema/instruction text (no actual goals, users, stories, or seed dimensions), state explicitly: "scorer payload missing — span input/output not populated", and pick `none_pass`. Do NOT try to evaluate from instructions alone.

---

## What you are evaluating

A seed has four dimensions. You must evaluate each one independently.
...
Be conservative: if you are unsure whether a criterion passes, mark it as FAIL. A false positive (calling a weak seed good) is worse than a false negative (calling a good seed weak) — a weak seed produces a bad dataset, and a bad dataset produces misleading evals.
